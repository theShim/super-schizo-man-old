    ...
    def draw_lines_3d(self, screen, lines):
        ...
        for i in range(0, len(all_xformed_pts) // 2):
            ...
            if p1 is not None and p2 is not None:
                # pygame.draw.line(screen, color, p1, p2, int(2 / 1/ camera.direction.z))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if keys_held[pygame.K_LEFT] or keys_held[pygame.K_RIGHT]:
            ...
            camera.direction.scale_to_length(1) if flag else ...
        ...
        xy = Vector2(camera.position.x, camera.position.y)
        ...
        if keys_held[pygame.K_q]:
            xy = xy + ms * view_xz.rotate(180)
        if keys_held[pygame.K_e]:
            xy = xy + ms * view_xz.rotate(0)
